...
//condition in line 41
// || self::isDevelopmentStore($session)



    // private static function isDevelopmentStore(Session $session): bool
    // {
    //     $shopDomain = $session->getShop();
    //     // Extract the shop name from the shop domain
    //     $shopName = explode(".", $shopDomain)[0];
    
    //     // Check if the shop name contains the phrase 'development' or 'dev'
    //     return strpos($shopName, 'development') !== false || strpos($shopName, 'dev') !== false;
    // }
    ...
    private static function requestPayment(Session $session, array $config)
    {
        ...
        $session = DbSession::where('shop',$shop)->first();
        $session->paid = $data['appSubscription']['status'];
        $session->save();
        
        return $data["confirmationUrl"];
    }

     static function requestRecurringPayment(Session $session, array $config, string $returnUrl): array
    {
