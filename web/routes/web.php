...
Route::get('/api/check-billing', function (Request $request) {
    $shop = $request->query('shop');
    $status = Session::where('shop',$shop)->value('paid');
       if($status!=='ACTIVE' && $shop!==null){
           \Log::info('if');
           $session = \Shopify\Utils::loadOfflineSession($shop);
           $confirmationUrl = EnsureBilling::check($session, Config::get('shopify.billing'));
...
Route::get('/api/auth/callback', function (Request $request) {
    ...
    $app_subscription = Registry::register('/api/webhooks', Topics::APP_SUBSCRIPTIONS_UPDATE , $shop, $session->getAccessToken());
    if ($app_subscription->isSuccess()) {
        Log::debug("Registered APP_SUBSCRIPTION_UPDATE  webhook for shop $shop");
    } else {
        Log::error(
...
Route::post('/api/export', [ExportController::class, 'export'])->middleware('shopify.auth');
Route::get('/api/scheduleData', [ExportController::class, 'scheduleData'])->middleware('shopify.auth');
Route::post('/api/delete', [ExportController::class, 'delete'])->middleware('shopify.auth');
Route::post('/api/status', [ExportController::class, 'status'])->middleware('shopify.auth');
